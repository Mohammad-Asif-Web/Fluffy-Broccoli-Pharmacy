...
// shopping
const productsArray = document.querySelector('#all-products-array');
const totalProductPrices = document.querySelector('#total-product-prices');
		...
		let result = productsInCart.map(product => {
			return ` <li class="buyItem row w-100 mb-4">
                        ...
                        <div class="col-5 item-text pl-3">
                            <h5 class="font-weight-bold" name="product-name">${product.name}</h5>
                            <p class="text-muted mb-0">3 each X 2</p>
                            <div class="count">
                                <button class="button-minus" data-id=${product.id}>-</button>
                                <span class="countOfProduct" name="product-qty">${product.count}</span>
                                <button class="button-plus" data-id=${product.id}>+</button> 
                            </div>
                        </div>
                        <div class="col-4 item-cost text-right">
                            <p class="mb-0 font-weight-bold h5" name="product-price">৳ ${(product.price * 10) / 10} </p>
                            <del>৳ 000</del>
                        </div>
                    </li>
                `
...
productsArray.setAttribute("value", JSON.stringify(productsInCart));
